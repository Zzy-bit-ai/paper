...
if not OPENROUTER_API_KEY:
    print("❌ Error: OPENROUTER_API_KEY not set! Please set it in .env or environment.")
    sys.exit(1)

print("✅ Using OpenRouter API for MiMo model")

# ==================== 参数 ====================
parser = argparse.ArgumentParser(description="KATE/Random Baseline for TriviaQA (MiMo API Version)")
...
# ==================== MiMo API 调用 (通过OpenRouter) ====================
def mimo_generate(prompt, model):
    ...
    for attempt in range(3):
        try:
            ...
            print(f"⚠️ MiMo attempt {attempt+1} failed: {e}")
            time.sleep(2 ** attempt)
    print("❌ MiMo failed after 3 attempts")
    return None
...
with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
    future_to_task = {executor.submit(mimo_generate, t['prompt'], args.model): t for t in tasks}
...
print(f"   MiMo errors: {error_count}")
